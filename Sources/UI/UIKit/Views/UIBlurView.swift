//
//  UIBlurView.swift
//  Espresso
//
//  Created by Mitch Treece on 7/2/18.
//

#if canImport(UIKit)

import UIKit
import SnapKit

// Thanks @aheze for the variable blur stuff :)
// https://github.com/aheze/VariableBlurView

/// Blurred `UIView` subclass that responds to tint color changes.
open class UIBlurView: UIBaseView {
    
    public enum Style {
        
        case system(UIBlurEffect.Style)
        
        case variable(maxRadius: CGFloat = 20,
                      mask: UIImage = UIBlurView.defaultVariableMask)
        
    }
    
    private var blurView: UIVisualEffectView?
    
    public var blurStyle: Style = .system(.systemMaterial) {
        didSet {
            setupBlurView()
        }
    }
    
    /// The blur view's content view.
    ///
    /// Add subviews to this, and not `UIBlurView` directly.
    public var contentView: UIView {
        return self.blurView!.contentView
    }
    
    open override func tintColorDidChange() {
        self.backgroundColor = self.tintColor
    }
    
    /// Initializes a new `UIBlurView` with a specified blur style.
    /// - parameter style: The blur style; _defaults to systemMaterial_.
    public init(style: Style = .system(.systemMaterial)) {
        
        self.blurStyle = style
        super.init(frame: .zero)
        
    }
    
    public required init?(coder: NSCoder) {
        super.init(coder: coder)
    }
    
    open override func willAppear() {
        
        super.willAppear()
        
        self.clipsToBounds = true
        
        setupBlurView()
        
    }

    // MARK: Private
    
    private func setupBlurView() {
        
        self.blurView?
            .removeFromSuperview()
        
        switch self.blurStyle {
        case .system(let style):
            
            self.blurView = UIVisualEffectView(effect: UIBlurEffect(style: style))

        case .variable(let maxRadius, let mask):
            
            self.blurView = UIVisualEffectView(effect: UIBlurEffect(style: .regular))
            
            if let filter = self.variableFilter,
               let cgMask = mask.cgImage {
                
                filter.setValue(maxRadius, forKey: "inputRadius")
                filter.setValue(cgMask, forKey: "inputMaskImage")
                filter.setValue(true, forKey: "inputNormalizeEdges")
                
                // Get rid of the visual effect view's dimming/tint view,
                // so we don't see a hard line.
                if self.blurView!.subviews.indices.contains(1) {
                    
                    let tintOverlayView = self.blurView!.subviews[1]
                    tintOverlayView.alpha = 0
                    
                }
                
                // We grab the visual effect view's `CABackdropLayer`,
                // so we can apply various real-time filters onto underneath views.
                let backdropLayer = self.blurView!.subviews.first?.layer

                // Replace the standard filters (i.e. `gaussianBlur`, `colorSaturate`, etc.)
                // with our variable bllur filter
                backdropLayer?.filters = [filter]
                
            }
            
        }
        
        self.addSubview(self.blurView!)
        self.blurView!.snp.makeConstraints { (make) in
            make.edges.equalToSuperview()
        }
        
    }
    
    // MARK: Variable Blur
    
    /// Variable blur type string.
    private let variableBlurTypeString = "variableBlur"
    
    /// Variable blur `CIFilter` class, base-64 encoded.
    private let _variableFilterClassString = "Q0FGaWx0ZXI="
    private lazy var variableFilterClassString: String = {
        
        guard let data = Data(base64Encoded: self._variableFilterClassString),
              let string = String(data: data, encoding: .utf8) else { return "" }
        
        return string
        
    }()
    
    /// Variable blur magic function, base-64 encoded.
    private let _variableFilterMethodString = "ZmlsdGVyV2l0aFR5cGU6"
    private lazy var variableFilterMethodString: String = {
        return ""
    }()
    
    /// Variable blur filter object.
    private lazy var variableFilter: NSObject? = {
        
        guard let filterClass = (NSClassFromString(self.variableFilterClassString) as AnyObject) as? NSObjectProtocol else {
            return nil
        }
        
        let method = Selector(self.variableFilterMethodString)
        
        guard filterClass
            .responds(to: method) else { return nil }
        
        let filter = filterClass
            .perform(method, with: self.variableBlurTypeString)
            .takeUnretainedValue()
        
        guard let filterObject = filter as? NSObject else { return nil }
        
        return filterObject
        
    }()
    
    /// The default variable mask image.
    ///
    /// The blur radius at each pixel depends on the alpha value of the corresponding pixel in the gradient mask.
    /// An alpha of 1 results in the max blur radius, while an alpha of 0 is completely unblurred.
    public static var defaultVariableMask: UIImage {
        
        guard let data = Data(base64Encoded: self.defaultVariableMaskString),
              let image = UIImage(data: data) else { return UIImage() }
        
        return image
        
    }
    
    private static let defaultVariableMaskString = """
    
    bjvf9+1Pfl3/GTX2ayFpmkPjAAAAAElFTkSuQmCC
    """
    
}

#endif
